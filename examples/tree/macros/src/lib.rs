...
impl Parse for ConstantGroup {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !content.is_empty() {
            ...
            } else if ident == "stack_frame_offset" {
                // stack_frame_offset!(NAME, Struct.field)
                // stack_frame_offset!(NAME, Struct.field[expr])
                // stack_frame_offset!(NAME, Struct.field[expr].inner_field)
                content.parse::<Token![!]>()?;
                let inner;
                syn::parenthesized!(inner in content);
                let base_name: Ident = inner.parse()?;
        ...
        match &c.kind {
            ...
            ConstantKind::StackFrameOffset {
                ...
                let (const_def, literal_repr) = gen_stack_frame_offset_code(
                    ...
                    field_path_tokens,
                    array_index,
                    &None, // constant_group! does not support align
                );
...
fn parse_group_params(content: ParseStream) -> syn::Result<(Option<String>, Option<syn::Expr>)> {
    let mut prefix = None;
    let mut align = None;

    // Parse header parameters (prefix and align) before constants.
    loop {
        ...
        if (ident == "prefix" || ident == "align") && fork.peek(Token![=]) {
            ...
            if ident == "prefix" {
                ...
                let align_expr: syn::Expr = content.parse()?;
                align = Some(align_expr);
            }
...
impl Parse for AsmConstantGroupInput {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        // Parse optional header parameters: prefix = "...", align = expr
        let (prefix, align) = parse_group_params(&content)?;
...
fn gen_stack_frame_offset_code(
    ...
    let align_assertion = match align {
        Some(align_expr) => quote! {
            assert!(
                result % (#align_expr as i64) == 0,
